// script.js — to'g'ri: PINFL 2..7 => DDMMYY, + Gender
...
function handleFile(file){
  ...
  reader.onload = function(e){
    ...
    // raw:false to prefer text reading
...
function processWorkbook(wb){
  ...
  // raw:false, defval:'' to keep text cells
  ...
  for (let i=0;i<json.length;i++){
    ...
    // if Excel stripped leading zeros / shortened, pad to 14
...
function parsePINFL(pinfl){
  // must be 14 digits after padding
  if (!/^\d{14}$/.test(pinfl)) return {birth:null, gender:null};

  const s = parseInt(pinfl[0], 10);        // century/jins code
  const dd = pinfl.slice(1,3);            // 2..3 -> day
  const mm = pinfl.slice(3,5);            // 4..5 -> month
  const yy = pinfl.slice(5,7);            // 6..7 -> year (two digits)

  // Determine century from S (1..6)
  let century;
  if (s === 1 || s === 2) century = 1800;
  else if (s === 3 || s === 4) century = 1900;
  else if (s === 5 || s === 6) century = 2000;
  else return {birth:null, gender:null};
...
function renderPreview(records){
  ...
  ['PINFL','Birthdate','Gender'].forEach(h=>{
...
function buildWorkbook(records){
  const ws = XLSX.utils.json_to_sheet(records, {header:['PINFL','Birthdate','Gender']});
